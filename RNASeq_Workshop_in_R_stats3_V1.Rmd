...
```{r}
lib.loc <- "/homes/homedirs8/workshops/190329-RNAseq/packages_r_3_5_0"
...
### STEP X: Load the following R packages into R by copying and pasting all the following at the terminal
...
### STEP X: Type the following into the console to identify the working current R working directory:
...
### STEP X: Loading the count data into R 
...
### STEP X: View the top of the "CountsTable_PPP2R3B" matrix object using the head command in R:
...
### STEP X: rlog trandformation and Scatter plots
...
### STEP X: Heatmap of sample-to-sample distances
...
### STEP X: Run the differential gene expression analysis in DESeq2:
...
### STEP X: plot dispersion estimates
...
### STEP X: Reporting the differential gene expression results
...
### STEP X: Writing the results to an excel file
...
### step X: Plot normalised counts for a single gene 
...
```{r}
plotCounts(DESeq2Experiment, intgroup = "condition", normalized = TRUE,
